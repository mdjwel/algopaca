...
_PRESETS: tuple[AiPreset, ...] = (
    ...
    AiPreset(
        ...
        summary="Calibrated macro & Gold/Silver Ratio playbook: buys pullbacks in confirmed gold uptrends across GLD, SLV & inverse hedge ETF (GLL), with 3-tier profit scaling (2.0R / 3.2R-4.0R / trailing runner) and wide ATR stops (1.6x GLD, 1.8x SLV).",
        min_confidence=0.70,
        # Calibrated risk management: 1.6 ATR stop for GLD (1.8 ATR cushion for SLV) gives breathing room without excessive loss.
        # Trailing stop armed at 2.0R ratchets to breakeven (Tier 1), take-profit at 3.2R (expands to 4.0R when ADX >= 28 or macro >= 1.5) (Tier 2),
        # and 50% runner trails for multi-week trend capture (Tier 3).
        atr_stop_mult=1.6,
        take_profit_r=3.2,
        trail_after_r=2.0,
        max_positions=2,
        risk_pct=1.8,
        instructions=(
            "Specialized Gold & Silver (GLD, SLV, GLL, IAU, PHYS) macro playbook.\n"
            ...
            "(b) Falling real yields (TIPS) and a stretched Gold/Silver ratio are the only macro factors with a "
            ...
            "GDX, GDXJ, DUST, UGL, GDXU, and GDXD are strictly excluded from this playbook to avoid equity beta, counterparty decay, and leveraged drag.\n"
            ...
            "3. Entry timing: prefer entering on a PULLBACK — RSI between 38 and 58 (or up to 65 in strong bull regimes with ADX >= 25 or macro >= 1.5), "
            "or price at or below SMA20 while the higher-timeframe trend stays up. Do NOT require price above EMA9 or a positive "
            "MACD histogram to go long; demanding short-term strength is the mean-reversion trap in (a).\n"
            "4. Vehicle: default to pure commodity vehicles (GLD, SLV, IAU, PHYS). "
            "GDX, GDXJ, DUST, UGL, GDXU, and GDXD are strictly excluded. "
            "SLV uses an automatic 1.8x ATR stop cushion to accommodate silver's higher volatility beta.\n"
            ...
            "size smaller than an equivalent long. Inverse ETF GLL expresses the bearish view without margin "
            "borrow by BUYING long (action='buy'). DUST, GDXD, and GDXU are strictly excluded. "
            ...
            "EXITS & RUNNERS:\n"
            "- Gold's edge is captured by holding trends, so the default answer on an open, working position is HOLD.\n"
            "- 3-Tier Profit Harvesting:\n"
            "  * Tier 1 (2.0R): Scale out 25% of position to lock initial profit and ratchet stop to breakeven (entry price).\n"
            "  * Tier 2 (3.2R - 4.0R): Scale out another 25% of position at take-profit target (dynamically expanded to 4.0R when ADX >= 28 or macro >= 1.5).\n"
            "  * Tier 3 (50% Runner): Let remaining 50% trail with the ATR trailing stop for multi-week trend capture.\n"
            "- INVERSE DECAY GUARD: For inverse ETF GLL, never hold longer than ~5 trading days or when inverse RSI reaches >= 65.0 to guard against leveraged volatility decay.\n"
            ...
            "position to CASH to avoid squeeze risk. Do NOT force an immediate long buy unless gold meets all technical pullback gates.\n"
            + _SHARED_EXIT
        ),
    ),
    AiPreset(
